...
export async function getRequests() {
  ...
  try {
    ...
    maintenanceContracts.status === 200 ||
      (errorMessage = maintenanceContracts.message);
    softwareServiceContracts.status === 200 ||
      (errorMessage = softwareServiceContracts.message);
    distributorRepresentativesContracts.status === 200 ||
      (errorMessage = distributorRepresentativesContracts.message);
    ...
    if (distributorRepresentativesContracts.status === 200) {
      allRequests = allRequests.concat(
        distributorRepresentativesContracts.data
      );
    }
  } catch (error) {
